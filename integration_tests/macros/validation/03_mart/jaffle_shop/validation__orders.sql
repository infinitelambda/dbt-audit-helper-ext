{# Validation config #}
{%- macro get_validation_config__orders() -%}
    ...
    {% set old_identifier = audit_helper_ext.get_old_identifier_name('orders') %}
...
{%- macro validation_count__orders() %}
    ...
    {% set validation_config = get_validation_config__orders() %}
    {% if execute %}
...
{%- macro validation_schema__orders() -%}
...
{# Row count by group #}
{%- macro validation_count_by_group__orders(group_by) %}
...
{%- macro validation_col__orders(columns_to_compare, summarize=true, limit=100) -%}
