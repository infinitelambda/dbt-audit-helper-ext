{# Validation config #}
{%- macro get_validation_config__sample_expressions_test() -%}

    {% set dbt_identifier = 'sample_expressions_test' %}
...
{# Row count #}
{%- macro validation_count__sample_expressions_test() %}

    {% set validation_config = get_validation_config__sample_expressions_test() %}
    {% if execute %}
...
{%- macro validation_schema__sample_expressions_test() -%}
...
{# Column comparison #}
{%- macro validation_all_col__sample_expressions_test(summarize=true) -%}
...
{%- macro validation_full__sample_expressions_test(summarize=true) -%}
...
{# Validations for All #}
{%- macro validations__sample_expressions_test(summarize=true) -%}
...
{# Row count by group #}
{%- macro validation_count_by_group__sample_expressions_test(group_by) %}
...
{%- macro validation_col__sample_expressions_test(columns_to_compare, summarize=true, limit=100) -%}
