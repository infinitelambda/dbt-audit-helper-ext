...
{# DuckDB has no `clone`; like Postgres it copies via drop + create-as-select. Unlike the Postgres #}
{# branch we omit the explicit begin/commit — dbt-duckdb already runs statements inside a managed #}
{# transaction, and DuckDB rejects nested transactions ("cannot start a transaction within a transaction"). #}
{% macro duckdb__clone_object(object_name, source_object_name, object_type, replace, dry_run, is_iceberg) %}
